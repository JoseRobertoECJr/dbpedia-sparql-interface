...
export class ConexoesComponent implements OnInit {
  ...
  search(id: number){
    ...
    this.dbpediaSparqlService.getSparQL(this.sparqlQuery).subscribe((data) => {
      ...
      console.log(data)
        const nodes = data.results.bindings.map((node, index) => {
          const indexx = index + this.lastIndex + 1
          return {
            id: indexx,
            label: node.value.type + "(" + node.value.value.split("/").pop().substring(0, 15) + ")",
            group: node.value.type.split("/").pop() }
        })

        console.log(this.nodes)

        const edges = data.results.bindings.map((node, index) => {
          const indexx = index + this.lastIndex + 1
        ...
        if(id == 0)
          nodes.push({ id: 0, label: "MAIN NODE", group: "MAIN NODE" })
  ...
  thatEx(){
    ...
    var edges = [
      { from: 1, to: 0, label: "label1", font: { size: 20, background: "white" } },
      { from: 2, to: 0 },
      { from: 4, to: 3 },
      { from: 5, to: 4 },
      { from: 4, to: 0 },
